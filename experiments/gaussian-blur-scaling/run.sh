...
for image in bricks leaf cheese; do
  for datatype in half4 float2 float; do
    for ipu in 1 2 4 8 16; do
      ...
      ./gb_low_level -n 100 -i ../../images/${image}.png -o /tmp/${image}-out.png --num-ipus ${ipu} --data-type ${datatype} --min-rows-per-tile 1 --min-cols-per-tile 1 >> gaussian-blur-scaling.csv 2>&1
    done
  done
done
